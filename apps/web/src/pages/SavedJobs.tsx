...
const mockJobs = [
  {
    ...
    notes: "Requires strong React skills.",
    status: "saved",
    deadline: "2024-12-31",
  },
  {
...
export default function SavedJobs() {
  ...
  const handleAddJob = (newJob: typeof empty) => {
    setJobs([
      { ...newJob, id: Math.max(...jobs.map((j) => j.id), 0) + 1 },
      ...jobs,
    ]);
    setOpen(false);
    setForm(empty);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Bookmark jobs manually or via the extension.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="border-border/50 bg-card/50 backdrop-blur-sm gap-2 h-10 hover:bg-muted/50"
            onClick={() =>
          ...
          <Button
            className="gradient-primary text-white border-0 glow-primary hover:opacity-90 h-10 px-5 shadow-lg"
          ...
          <div className="relative flex flex-col md:flex-row md:items-center gap-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-xl leading-relaxed">
                1-click save any job from LinkedIn, Unstop, Internshala, or any
                company site directly to your EngiBuddy tracker.
              </p>
              <div className="flex flex-wrap gap-2 mt-3">
                {[
                  ...
                  <Badge
                    key={s}
                    variant="outline"
                    className="text-[10px] font-semibold tracking-wider border-primary/20 text-primary/80 bg-primary/5 uppercase"
                  >
                    {s}
                  </Badge>
                ))}
            ...
            <Button
              variant="outline"
              className="border-primary/30 text-primary hover:bg-primary/10 mt-4 md:mt-0 w-full md:w-auto h-10 px-6 font-semibold shadow-sm"
            >
              Notify Me
            </Button>
          </div>
      ...
      {filtered.length === 0 ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex flex-col items-center justify-center py-20 px-6 text-center border border-border/50 rounded-2xl bg-card/30 backdrop-blur-sm"
        >
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-[300px] leading-relaxed">
            {jobs.length === 0
              ? "Save jobs manually or install the browser extension to start building your pipeline."
              ...
              >
                ...
                <div className="relative">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-heading font-bold text-base leading-tight flex-1 pr-3 text-foreground">
                      {job.title}
                    </h3>
                    <button
                      onClick={() => handleDeleteJob(job.id)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-rose-400 p-1.5 hover:bg-rose-500/10 rounded-lg flex-shrink-0"
                    >
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    <Badge
                      variant="outline"
                      className={`text-[10px] font-semibold uppercase tracking-wider ${TYPE_COLORS[job.type]}`}
                    >
                      {job.type}
                    </Badge>
                    <Badge
                      variant="outline"
                      className={`text-[10px] font-semibold uppercase tracking-wider ${STATUS_COLORS[job.status]}`}
                    >
                      {job.status}
                    </Badge>
                    {job.work_mode && (
                      <Badge
                        variant="outline"
                        className="text-[10px] font-semibold uppercase tracking-wider border-border/50 bg-muted/50"
                      >
                        {job.work_mode}
                      </Badge>
                    )}
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-border/50 relative">
                  {job.status === "saved" && (
                    <Button
                      size="sm"
                      className="h-9 font-semibold gradient-primary text-white border-0 flex-1 shadow-sm hover:opacity-90 transition-opacity"
                      onClick={() =>
                        handleUpdateJob(job.id, { status: "applied" })
                      }
                    >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 mt-2">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent className="border-border/50 bg-background/95 backdrop-blur-xl">
                    {["internship", "job", "hackathon", "competition"].map(
                      (t) => (
            ...
            <Button
              className="w-full gradient-primary text-white border-0 mt-4 h-10 shadow-lg glow-primary"
