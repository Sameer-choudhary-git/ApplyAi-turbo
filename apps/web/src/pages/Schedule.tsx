...
const mockApplications = [
  ...
  },
  {
    ...
    company: "Meta",
    interview_date: new Date(Date.now() + 259200000).toISOString(),
    deadline: null,
  },
];
...
export default function Schedule() {
  ...
  const getEventsForDay = (day) => {
    const dayInterviews = interviews.filter((a) =>
      isSameDay(new Date(a.interview_date), day),
    );
    const dayDeadlines = deadlines.filter((a) =>
  ...
  const upcomingEvents = [...interviews, ...deadlines]
    ...
    .sort(
      (a, b) =>
        new Date(a.interview_date || a.deadline).getTime() -
        new Date(b.interview_date || b.deadline).getTime(),
    )
    .slice(0, 6);
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Interviews, deadlines, and important dates.
          </p>
        </div>
      </div>
            ...
            <CardContent className="p-6">
              ...
              <div className="flex items-center justify-center gap-6 mt-8 pt-4 border-t border-border/50 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                <span className="flex items-center gap-2">
                  <span className="w-4 h-1.5 rounded-full bg-accent" />{" "}
                  Interview
                </span>
                <span className="flex items-center gap-2">
                  <span className="w-4 h-1.5 rounded-full bg-amber-400" />{" "}
                  Deadline
                </span>
              </div>
            </CardContent>
                    ...
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          >
                            {format(
                              new Date(e.interview_date || e.deadline),
                              "MMM d, yyyy • h:mm a",
                            )}
                          </p>
                        </div>
