...
export default function Analytics({ applications = [], isLoading = false }) {
  ...
  const avgProb = withProb.length > 0
    ? Math.round(withProb.reduce((s, a) => s + a.success_probability, 0) / withProb.length)
    : 0;
  ...
  const typeLabelMap = { job: 'Full-Time', internship: 'Internship', hackathon: 'Hackathon' };
  ...
  const positiveStatuses = new Set(['shortlisted', 'interview_scheduled', 'accepted']);
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden h-full">
            ...
            <CardContent className="p-5">
              {pieData.length > 0 ? (
                ...
                <div className="h-[240px] flex items-center justify-center text-sm text-muted-foreground font-medium">No data yet</div>
              )}
            </CardContent>
          </Card>
        </motion.div>
              ...
              {typeData.length > 0 ? (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={typeData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: 'rgba(255,255,255,0.5)' }} stroke="transparent" />
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden h-full">
            ...
            <CardContent className="p-6 space-y-5">
              {platformData.length > 0 ? platformData.map((p, i) => (
                ...
                <div className="h-[200px] flex items-center justify-center text-sm text-muted-foreground font-medium">No data yet</div>
              )}
            </CardContent>
          </Card>
        </motion.div>
