...
const groups: Array<{ label: string; items: NavItem[] }> = [
  {
    ...
    items: [
      ...
      { path: "/greenhouse", icon: Search, label: "Greenhouse jobs" },

      { path: "/saved-jobs", icon: Bookmark, label: "Saved jobs" },
      { path: "/job-skill", icon: Sparkles, label: "Job Skill" },
      { path: "/plans", icon: CreditCard, label: "Plans & services" },
    ...
    >
      {isActive && (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-sidebar-accent shadow-[inset_0_1px_0_hsl(var(--foreground)/.04)]"
      ...
      <Icon
        className={cn(
          ...
          isActive
            ? "text-primary"
            : "text-sidebar-foreground/42 group-hover:text-sidebar-foreground/75",
        )}
...
export default function Sidebar({ collapsed, setCollapsed }: SidebarProps) {
  return (
    ...
    >
        ...
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.div
              ...
              className="min-w-0"
            >
              <p className="font-heading text-[15px] font-extrabold tracking-[-0.03em] text-sidebar-foreground">
                Apply AI
              </p>
              <p className="mt-0.5 truncate text-[11px] font-medium text-sidebar-foreground/42">
      ...
      <div
        className={cn(
          "mt-8 flex-1 overflow-y-auto pb-4",
          collapsed ? "px-0" : "px-1",
          "scrollbar-subtle",
        )}
      >
      ...
      >
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 6 }}
              className="mb-3 rounded-2xl border border-primary/15 bg-primary/[0.07] p-3"
            >
              ...
              <p className="mt-2 text-[11px] leading-5 text-sidebar-foreground/45">
                Your applications, follow-ups, and next steps in one place.
              </p>
              <Link
                to="/analytics"
                className="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-primary hover:text-sidebar-foreground"
              >
        ...
        <button
          ...
          className={cn(
            "flex h-10 w-full items-center rounded-xl text-xs font-semibold text-sidebar-foreground/45 hover:bg-sidebar-accent/60 hover:text-sidebar-foreground",
